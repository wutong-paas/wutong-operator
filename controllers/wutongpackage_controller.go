...
// 	err := r.Client.Get(cancleCtx, request.NamespacedName, wutongpackage)
// 	if err != nil {
...
// 	if err := r.Client.Get(cancleCtx, types.NamespacedName{Namespace: wutongpackage.Namespace, Name: constants.WutongClusterName}, cluster); err != nil {
...
// 		wutongpackage.Status = initPackageStatus(wutongv1alpha1.Completed)
// 		if err := updateCRStatus(r.Client, wutongpackage); err != nil {
...
// 	updateStatus, re := checkStatusCanReturn(wutongpackage)
// 	if updateStatus {
// 		if err := updateCRStatus(r.Client, wutongpackage); err != nil {
...
// 	pkg, err := newPackageInfo(cancleCtx, r.Client, wutongpackage, cluster, log)
// 	if err != nil {
// 		if pkg != nil {
// 			pkg.updateConditionStatus(wutongv1alpha1.Init, wutongv1alpha1.Failed)
// 			pkg.updateConditionReason(wutongv1alpha1.Init, err.Error(), "create package handle failure")
// 			_ = pkg.updateCRStatus()
// 		}
...
// func checkStatusCanReturn(wutongpackage *wutongv1alpha1.WutongPackage) (updateStatus bool, re *reconcile.Result) {
// 	if len(wutongpackage.Status.Conditions) == 0 {
// 		wutongpackage.Status = initPackageStatus(wutongv1alpha1.Waiting)
// 		return true, &reconcile.Result{}
// 	}
// 	completedCount := 0
// 	for _, cond := range wutongpackage.Status.Conditions {
...
// 		}
// 	}
// 	if completedCount == len(wutongpackage.Status.Conditions) {
// 		return false, &reconcile.Result{}
// 	}
// 	return false, nil
// }
...
// func newPackageInfo(ctx context.Context, client client.Client, p *wutongv1alpha1.WutongPackage, cluster *wutongv1alpha1.WutongCluster, reqLogger logr.Logger) (*packageInfo, error) {
...
// func (p *packageInfo) configByCluster(c *wutongv1alpha1.WutongCluster) error {
...
// func (p *packageInfo) updateCRStatus() error {
...
// func (p *packageInfo) checkClusterConfig() error {
// 	cluster := p.cluster
...
// func (p *packageInfo) findCondition(typ3 wutongv1alpha1.PackageConditionType) *wutongv1alpha1.PackageCondition {
...
// func (p *packageInfo) updateConditionStatus(typ3 wutongv1alpha1.PackageConditionType, status wutongv1alpha1.PackageConditionStatus) {
// 	for i, condition := range p.pkg.Status.Conditions {
// 		if condition.Type == typ3 {
...
// func (p *packageInfo) updateConditionReason(typ3 wutongv1alpha1.PackageConditionType, reason, message string) {
// 	for i, condition := range p.pkg.Status.Conditions {
// 		if condition.Type == typ3 {
// 			p.pkg.Status.Conditions[i].LastHeartbeatTime = metav1.Now()
// 			p.pkg.Status.Conditions[i].Reason = reason
...
// func (p *packageInfo) updateConditionProgress(typ3 wutongv1alpha1.PackageConditionType, progress int32) bool {
// 	if progress > 100 {
// 		progress = 100
// 	}
// 	for i, condition := range p.pkg.Status.Conditions {
...
// func (p *packageInfo) completeCondition(con *wutongv1alpha1.PackageCondition) error {
// 	if con == nil {
// 		return nil
// 	}
...
// func (p *packageInfo) runningCondition(con *wutongv1alpha1.PackageCondition) error {
// 	if con == nil {
// 		return nil
// 	}
...
// func (p *packageInfo) canDownload() bool {
...
// func (p *packageInfo) canUnpack() bool {
// 	if con := p.findCondition(wutongv1alpha1.DownloadPackage); con != nil {
...
// func (p *packageInfo) canPushImage() bool {
// 	if uncon := p.findCondition(wutongv1alpha1.UnpackPackage); uncon != nil {
// 		if uncon.Status != wutongv1alpha1.Completed {
// 			return false
// 		}
...
// func (p *packageInfo) canReady() bool {
...
// func (p *packageInfo) setInitStatus() error {
...
// func (p *packageInfo) donwnloadPackage() error {
...
// 		p.updateConditionReason(wutongv1alpha1.Init, err.Error(), "download wutong package error, will retry")
// 		_ = p.updateCRStatus()
...
// // handle
// func (p *packageInfo) handle() error {
...
// 		p.updateConditionReason(wutongv1alpha1.Init, err.Error(), "get wutong cluster config failure")
// 		_ = p.updateCRStatus()
// 		return err
// 	}
...
// 		p.updateConditionReason(wutongv1alpha1.Init, err.Error(), "set init status failure")
// 		_ = p.updateCRStatus()
// 		return err
// 	}
// 	if p.canDownload() {
...
// 			p.updateConditionReason(wutongv1alpha1.DownloadPackage, err.Error(), "download package failure")
// 			_ = p.updateCRStatus()
...
// 			p.updateConditionReason(wutongv1alpha1.UnpackPackage, err.Error(), "unpack package failure")
// 			_ = p.updateCRStatus()
...
// 				p.updateConditionReason(wutongv1alpha1.PushImage, err.Error(), "pull and push images failure")
// 				_ = p.updateCRStatus()
...
// func (p *packageInfo) untartar() error {
...
// func (p *packageInfo) imagePullAndPush() error {
...
// func (p *packageInfo) imagePush(image images.Image) error {
...
// func (p *packageInfo) isImageRepositoryReady() bool {
